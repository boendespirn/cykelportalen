...
- **`agent_catalog.py`** — det autoritative katalog over, hvad der må køres, og hvilke kommandoer hvert `job_key` svarer til. Ligger i kode, ikke i databasen: runneren skal alligevel have en hardkodet allowlist, og to kilder til sandhed ville komme ud af trit. **Fra nettet kan man kun vælge hvilket forudgodkendt job, hvilket løb og hvilken etape — aldrig hvad der køres.** Indfør aldrig et felt, hvor kommandoen kommer udefra.
  ...
  - **Jobs (2026-09-09):** `startliste`, `raesinfo` (etapedata → PCS-profilbilleder → egen hel-etape-højdeprofil → stigningsrækker → ASO-roadbook → TV-tider), `stigningsprofiler` (VeloViewer), `rytterstats`, `fuld_forberedelse`, `resultater` (resultat + klassement → etapereferat), `raesreferat` + de løbsløse `nyheder_rss`, `nyheder_ai`, `tv_tider`, `daglig_pipeline`. `LEGACY_LABELS` holder navnene på de udgåede job, så historikken stadig kan læses.
  - **Etapereferatet er ikke et selvstændigt job.** Det er trin 2 i `resultater` og kan derfor ikke køres uden at resultat og klassement er hentet i samme kørsel — referatet grundfæster sig i dem, og en knap for sig gjorde det muligt at skrive et referat oven på forældede resultater.
...
- **`data_sources.py`** — svarer paa "hvad KAN overhovedet hentes for dette loeb?", til forskel fra `race_completeness.py`, der svarer paa "hvad HAR vi hentet?". Fem kilder: PCS-loebsside (`races.pcs_url`), PCS-etapesider (`stages.pcs_stage_url`), PCS-live-tidslinje (findes kun for etapeloeb — endagsloebs `/live` svarer 403, afproevet 2026-09-09), GPX (`CYCLINGSTAGE_GPX_PAGES`) og ASO-roadbook (`ASO_SITES`). Kilde-noeglerne matcher `STEP_SOURCES` i `agent_catalog.py`, og det er dét, der lader dashboardet skrive "springer trin 3 over — GPX mangler" i stedet for at lade agenten stoppe stille. Statussen `ikke_relevant` (fx ASO paa et ikke-ASO-loeb) holdes adskilt fra `nej`: en advarsel, der altid staar der, holder man op med at laese. Agent-moduler importeres gennem `_agent_attr()`, som saetter `sys.stdout` tilbage bagefter — agenterne pakker den ind ved import, og uden det kunne et kildeopslag goere api.py stum.
...
## Referater — pr. etape og for hele ræset
...
- **`agents/race_recap_agent.py`** — skriver `races.race_recap`: ÉN samlet beretning om hvordan hele ræset forløb, vist på løbssiden (`/{slug}`), ikke på en etapeside. Kilden er udelukkende vores egen verificerede database — etapereferaterne, etapevinderne og de endelige klassementer — så agenten kan ikke opfinde en hændelse, der ikke allerede er kontrolleret. Kører først når `end_date` er passeret (`--force` omgår det og skriver det i loggen), kræver at mindst 80% af de kørte etaper har et referat, og springer over hvis der allerede er et, medmindre `--force`. Job: `raesreferat` (Efter løbet).
- **`agents/historic_recap_agent.py`** — **ude af dashboardet 2026-09-09.** Skriver `stages.historic_recap` pr. etape for tidligere sæsoner ud fra TourTracker. Den gjorde reelt det samme som `stage_recap_agent.py`, bare for historiske løb, og knappen blev taget ud til fordel for `raesreferat`. Filen ligger stadig på disken og kan køres manuelt til backfill af gamle sæsoner.
